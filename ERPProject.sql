...
#--01---Financial Transactions Above a Certain Amount
select record_id, record_date, transaction_amount, record_type
from Financials
where transaction_amount > 1200;

#--02---Total Revenue Generated by Each Client
select c.buyer_name, sum(s.total_price) as total_revenue
from clients c
join Sales s on c.buyer_id = s.buyer_id
group by c.buyer_name;

#--03--- Financial Transactions of a Specific Employee
select record_id, record_date, transaction_amount, record_type, details
from Financials
where Emp_id = 2;

#--04---Sales Made Using a Specific Payment Type
select s.transaction_id, s.transaction_date, s.total_price, s.payment_type
from Sales s
...
select e.first_name, e.last_name, SUM(s.total_price) AS total_sales
from Employees e
join Sales s on e.Emp_id = s.handled_by
where e.Emp_id = 1
...
select i.item_name, i.item_category, v.vendor_name
from Inventory i
join vendors v on i.vendor_id = v.vendor_id;

#--09---Sales Transactions for a Specific Client
select s.transaction_id, s.transaction_date, i.item_name, s.quantity_purchased, s.total_price
from Sales s
